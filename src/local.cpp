...
char filename[] = "output_local.bmp";
...
int main(int argc, char* argv[])
{
	...
	for (int y=0; y<imageHeight; ++y)
	{
		for (int x=0; x<imageWidth; ++x)
		{
			...
			{
				float w = 2.7;
				float startx = 0.6, endx = 0.8;
				float starty = 0.79, endy = 0.99;
				vec3 tar(-w/2 + w*(startx + (endx-startx)*(x+randf())/imageWidth),
				1.0+w/2-w*(starty + (endy - starty)*(y+randf())/imageHeight), 0);
